...
/* ============================================================
   ...
   No server, no cloud: pages, photos and details never leave home.
   ============================================================ */
...
const MILESTONES = [
  { key: 'home-test', type: 'milestone', week: 4, title: 'The little line', note: 'Two lines. The start of you. We kept the test that night, hardly believing it — then we told no one yet, just the two of us, smiling at it in the dark.', icon: '🍀', mentions: [] },
  { key: 'beta-hcg', type: 'milestone', week: 5, title: 'Your first blood test', note: 'Our first beta-hCG came back, and then a second draw a couple of days later, watching your numbers double the way they should. Our first proof that you were really here.', icon: '🩸', mentions: [{ key: 'hcg1', label: 'Beta-hCG blood test #1', note: '' }, { key: 'hcg2', label: 'Beta-hCG blood test #2 (48 hr repeat)', note: '' }] },
  { key: 'sac', type: 'milestone', week: 5, title: 'We saw the sac', note: 'On the first scan, there was a tiny gestational sac in the right place. Hearing the words "it is exactly where it should be" — such relief, such quiet magic.', icon: '🥚', mentions: [] },
  { key: 'heartbeat', type: 'milestone', week: 6, title: 'We heard your heartbeat', note: 'A small, quick flicker appeared on the screen and they let us listen. The most beautiful little rhythm, all your own.', icon: '💓', mentions: [] },
  { key: 'booking', type: 'milestone', week: 8, title: 'Our booking appointment', note: 'The first big meeting with our midwife — how far along we are, the check-ups, the what-if list, and all the little formalities that made you feel wonderfully, officially expected.', icon: '📋', mentions: [{ key: 'booking-panel', label: 'Booking bloods — blood group, full blood count, anaemia & infection screen', note: '' }, { key: 'urine', label: 'Urine dipstick & blood pressure', note: '' }] },
  { key: 'dating', type: 'milestone', week: 12, title: 'Our dating scan', note: 'Suddenly you looked like a tiny person, arms and legs waving. This scan gave us a real due date and the first proper look at who you might be.', icon: '🫧', mentions: [{ label: 'Combined screening blood test (10–14 wks)', note: '' }, { label: 'Nuchal translucency measurement', note: '' }] },
  { key: 'movements', type: 'milestone', week: 19, title: 'We felt you move', note: 'The first flutter, so faint I almost missed it. Becomes the way we say goodnight — a little kick from you, a hand from us.', icon: '🦋', mentions: [] },
  { key: 'anatomy', type: 'milestone', week: 20, title: 'The 20-week scan', note: 'Ten tiny fingers, ten toes, a perfect little heart. And somewhere in those measurements, the doctors tell us everything is on its way — the most wonderful report we have ever received.', icon: '🏵️', mentions: [{ label: 'Quad screening blood test (if not done at 12 wks)', note: '' }] },
  { key: 'glucose', type: 'milestone', week: 26, title: 'The sugar-check appointment', note: 'A too-sweet drink, an hour of waiting, and then the two little vials that tell the midwife everything she needs to know.', icon: '🧪', mentions: [{ label: 'Oral glucose tolerance test (GTT)', note: '' }] },
  { key: 'growth', type: 'milestone', week: 32, title: 'A growth check', note: 'Measuring, measuring — how long, how heavy, how proportioned. You are on track and we are so proud of you, and you have not even arrived yet.', icon: '📏', mentions: [{ label: 'Repeat full blood count (anaemia check)', note: '' }, { label: 'Anti-D (if Rhesus negative)', note: '' }] },
  { key: 'turned', type: 'milestone', week: 34, title: 'You turned head-down', note: "You have decided to get ready. A little knock from the inside tells me you're settling into place for the big entrance.", icon: '🔄', mentions: [] },
  { key: 'fullterm', type: 'milestone', week: 37, title: 'Full term', note: 'Officially "on the right side of the numbers" — you could arrive at any moment. We have been ready for you for a very long time.', icon: '🎈', mentions: [] },
  { key: 'birth', type: 'milestone', week: 40, title: 'Your birth day', note: 'The day the story of waiting comes to its very first ending — the page where everything becomes real, soft and perfect.', icon: '👶', mentions: [] }
];
...
/* A warm growth card for a single week (4–40), built from the shared guide
   content (window.PREGNANCY_GUIDE): baby growth, common symptoms, feelings
   and a cute analogy. Returns null when the week has no guide data. */
function weekCardFor(w) {
  const g = window.PREGNANCY_GUIDE && window.PREGNANCY_GUIDE.weeks && window.PREGNANCY_GUIDE.weeks[w];
  if (!g) return null;
...
function milestoneEntry(m, n, base) {
  return {
    id: 'milestone-' + m.key + '-' + Math.random().toString(16).slice(2, 8),
    type: m.type,
    date: '',
    title: m.title,
    note: m.note,
...
function weekCardEntry(w, n, base) {
  ...
  if (!c) return null;
  return {
    id: 'week-' + w + '-' + Math.random().toString(16).slice(2, 8),
    type: 'milestone',
    date: '',
    title: c.title,
    note: c.note,
...
async function idbPut(store, value) {
  const d = await openDB();
  return idbReq(d.transaction(store, 'readwrite').objectStore(store).put(value));
}
...
/* --- validation / sorting (mirrors the original server behaviour) --- */
function safeText(v) {
...
function normalizeEntry(body) {
  ...
  return {
    ...
    arrow: body.arrow === '1' || body.arrow === 'true' || body.arrow === true,
    week: w,
    trimester: w <= 13 ? 1 : w <= 26 ? 2 : 3,
...
function rankKey(e) {
  if (e.date) {
    const w = weeksAt(e.date);
    if (w) return { t: 0, week: w.week, date: e.date, created: e.createdAt || 0 };
...
/* One-time migration: lock the current order of any pre-sortOrder pages. */
async function migrateSortOrders() {
  const all = await idbAll('entries');
  const todo = all.filter((e) => typeof e.sortOrder !== 'number').sort(compareRank);
  if (!todo.length) return;
  const pool = all.filter((e) => typeof e.sortOrder === 'number');
  for (const e of todo) {
    e.sortOrder = placeNew(e, pool);
    pool.push(e);
  }
  for (const e of todo) await idbPut('entries', e);
}
...
/* One-time repair: an older bug could exile a dated milestone to the very end
   ...
   every page to its natural timeline order once. Newer builds skip this via the
   flag, so edits keep their place from then on. */
async function repairOrderV1() {
  let flag = false;
  try { flag = localStorage.getItem('pj-order-repair-v1') === '1'; } catch {}
  if (flag) return;
  const all = await idbAll('entries');
  if (!all.length) { try { localStorage.setItem('pj-order-repair-v1', '1'); } catch {} return; }
  const sorted = all.slice().sort(compareRank);
  const pool = [];
  ...
  try { localStorage.setItem('pj-order-repair-v1', '1'); } catch {}
}

/* One-time: fill an existing journal with a growth card for every week 4–40
   (plus any missing milestone), slotting each new page into its natural spot.
   Newer builds skip this via the flag, so deletions stay deleted. */
async function restoreWeekCardsV1() {
  let flag = false;
  try { flag = localStorage.getItem('pj-weekcards-v1') === '1'; } catch {}
  if (flag) return;
  const all = await idbAll('entries');
  if (!all.length) { try { localStorage.setItem('pj-weekcards-v1', '1'); } catch {} return; }
  const haveWeeks = new Set(all.filter((e) => e.weekCard).map((e) => e.week));
  const haveTitles = new Set(all.map((e) => e.title.trim()));
  const base = Date.now();
  ...
  try { localStorage.setItem('pj-weekcards-v1', '1'); } catch {}
}

/* --- data access --- */
async function loadSettings() {
  const rec = await idbGet('settings', 'main');
  return { ...DEFAULT_SETTINGS, ...(rec && rec.value ? rec.value : {}) };
}

async function persistSettings(value) {
  await idbPut('settings', { key: 'main', value });
  return value;
}

async function listEntries() {
  return sortEntries(await idbAll('entries'));
}

const photoCache = new Map(); // photoId -> { blob, url }

async function resolvePhoto(entry) {
  if (!entry.photoId) { entry._photoUrl = null; return; }
  let rec = photoCache.get(entry.photoId);
  if (!rec) {
    const row = await idbGet('photos', entry.photoId);
    if (row && row.blob) { rec = { blob: row.blob, url: null }; photoCache.set(entry.photoId, rec); }
  }
  if (rec) {
    if (!rec.url) rec.url = URL.createObjectURL(rec.blob);
    entry._photoUrl = rec.url;
  } else {
    entry._photoUrl = null;
  }
}

async function resolvePhotos(list) {
  for (const e of list) await resolvePhoto(e);
}

async function deletePhoto(photoId) {
  const rec = photoCache.get(photoId);
  if (rec) {
...
async function saveEntry(entry, photoBlob) {
  if (photoBlob) {
    const photoId = crypto.randomUUID();
    await idbPut('photos', { id: photoId, blob: photoBlob });
    entry.photoId = photoId;
  }
  await idbPut('entries', entry);
}

async function updateEntry(entry, photoBlob, removePhoto) {
  const old = await idbGet('entries', entry.id);
  if (old && typeof old.sortOrder === 'number') entry.sortOrder = old.sortOrder; // editing never moves a page
  if (old && !entry.week && old.week) entry.week = old.week; // keep the week hint if a dated page was edited
  const oldPhotoId = old && old.photoId;
  if (photoBlob) {
    const photoId = crypto.randomUUID();
    await idbPut('photos', { id: photoId, blob: photoBlob });
    entry.photoId = photoId;
...
async function ensureTemplate() {
  if ((await idbAll('entries')).length > 0) return;
  for (const e of templateEntries()) await idbPut('entries', e);
}

async function loadAll() {
  settings = await loadSettings();
  await migrateSortOrders();
  await repairOrderV1();
  await restoreWeekCardsV1();
  entries = await listEntries();
  await resolvePhotos(entries);
}

/* --- backup: export / import (settings + pages + photos as one JSON file) --- */
...
async function exportBackup() {
  const allEntries = await idbAll('entries');
  const allPhotos = await idbAll('photos');
  const photos = [];
  for (const p of allPhotos) {
    if (p && p.blob) photos.push({ id: p.id, dataUrl: await blobToDataURL(p.blob) });
  }
  const payload = {
    app: 'journey-journal',
  ...
  a.href = url;
  a.download = 'our-journey-backup-' + new Date().toISOString().slice(0, 10) + '.json';
  a.click();
...
async function importBackup(file) {
  let obj;
  try { obj = JSON.parse(await file.text()); } catch { throw new Error('That file is not a valid backup.'); }
  const entriesArr = Array.isArray(obj) ? obj : Array.isArray(obj.entries) ? obj.entries : null;
  const photosArr = !Array.isArray(obj) && Array.isArray(obj.photos) ? obj.photos : [];
  const settingsObj = !Array.isArray(obj) && obj.settings && typeof obj.settings === 'object' ? obj.settings : null;
  if (!entriesArr) throw new Error('No pages found in that file.');

  const existing = await idbAll('entries');
  for (const e of existing) { if (e.photoId) await deletePhoto(e.photoId); await idbDelete('entries', e.id); }
  for (const p of photosArr) {
    if (p && p.id && p.dataUrl) await idbPut('photos', { id: p.id, blob: dataURLToBlob(p.dataUrl) });
  }
  for (const e of entriesArr) {
    if (!e || !e.id) continue;
    if (!e.type) e.type = 'note';
    if (!Array.isArray(e.mentions)) e.mentions = [];
    if (!e.photoId) e.photoId = null;
    if (!e.createdAt) e.createdAt = Date.now();
    await idbPut('entries', e);
  }
  if (settingsObj) await persistSettings({ ...DEFAULT_SETTINGS, ...settingsObj });
  for (const rec of photoCache.values()) { if (rec.url) URL.revokeObjectURL(rec.url); }
  photoCache.clear();
}
...
function applyTheme(themeId) {
  ...
  const warmBg = getComputedStyle(document.documentElement).getPropertyValue('--warm-bg').trim();
  const meta = document.querySelector('meta[name="theme-color"]');
  if (meta && warmBg) meta.setAttribute('content', warmBg);
}
...
function polaroidHTML(entry) {
  ...
  return `
    <figure class="polaroid">
      ${tape}
      <img src="${escapeHtml(entry._photoUrl || '')}" alt="${escapeHtml(entry.title || entry.photoCaption || 'A memory')}" loading="lazy" />
...
function pageHTML(entry) {
  ...
  const hasPhoto = !!entry._photoUrl;
...
async function reorderEnd(e) {
  ...
  try {
    for (const x of entries) await idbPut('entries', x);
...
function openEditor(entry) {
  ...
  if (entry && entry._photoUrl) {
    $('#dz-img').src = entry._photoUrl;
...
async function saveEditor() {
  const fields = {
    ...
    photoSize: chosenSize,
    photoCaption: $('#f-photocaption').value.trim(),
    week: $('#f-week').value || '',
    mentions: collectMentions(),
    icon: chosenIcon,
    arrow: $('#f-arrow').checked
  };
  ...
  try {
    ...
    if (editingId) {
      ...
      entry.id = editingId;
      entry.createdAt = old ? old.createdAt : Date.now();
      await updateEntry(entry, pendingPhoto ? pendingPhoto.file : null, removePhotoFlag);
      toast('This page is saved. 💛');
    } else {
      entry.id = crypto.randomUUID();
      entry.createdAt = Date.now();
      entry.sortOrder = placeNew(entry, entries);
      await saveEntry(entry, pendingPhoto ? pendingPhoto.file : null);
...
function openSettings() {
  ...
  // once an effective due date exists we never ask for the period date again
...
/* Builds a fully self-contained, print-ready HTML document of the whole
   ...
   (blobToDataURL is shared with the backup feature above.) */
...
function wire() {
  ...
  $('#settings-export').addEventListener('click', () => {
    exportBackup()
      .then(() => toast('Backup saved — keep it somewhere safe. 💾'))
  ...
  $('#settings-import-file').addEventListener('change', async (e) => {
    ...
    if (!file) return;
    if (!confirm('Importing a backup replaces all current pages and photos. Continue?')) { e.target.value = ''; return; }
    try {
...
(async function init() {
  ...
  if (typeof crypto === 'undefined' || typeof crypto.randomUUID !== 'function') {
    toast('This journal needs a secure connection (https:// or localhost) to work.');
  }
  try {
    await openDB();
    if (navigator.storage && navigator.storage.persist) {
      navigator.storage.persist().catch(() => {}); // ask the OS not to evict our journal
    }
    await ensureTemplate();
    await loadAll();
  } catch (err) {
    toast("Could not open this journal's storage. Is this page being served over http(s)?");
  }
  applyTheme();
  renderAll();
  if (window.BlossomChat) {
